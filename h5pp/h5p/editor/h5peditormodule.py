...
def getUserData(contentId, subContentId, dataId, userId):
	result = h5p_content_user_data.objects.filter(user_id=userId, content_main_id=contentId, sub_content_id = subContentId, data_id=dataId).values()
...
def deleteUserData(contentId, subContentId, dataId, userId):
	h5p_content_user_data.objects.filter(user_id=userId, content_main_id=contentId, sub_content_id=subContentId, data_id=dataId).delete()
...
def getLibraryProperty(library, prop='all'):
	...
	if matches:
		...
		if prop == 'all':
			...
			temp = h5p_libraries.objects.filter(machine_name=libraryData['machineName'], major_version=libraryData['majorVersion'], minor_version=libraryData['minorVersion']).values('library_id')
			return temp
		else:
