...
class H5PDjangoEditor:
	...
	def __init__(self, h5p, storage, basePath, filesDir, editorFilesDir=None):
		...
		self.editorFilesDir = (filesDir if editorFilesDir == None else editorFilesDir) + 'editor'
		...
		if 'styles' in files:
			for css in files['styles']:
				if re.search('/:\/\//', css['path']):
					...
					libraryData['css'][url + css['path'] + css['version']] = re.sub('(?i)url\([\']?(?![a-z]+:|\/+)([^\')]+)[\']?\)', self.buildCssPath, self.h5p.fs.getContent(css['path']))

		# Add translations for libraries
		for key, library in libraries.iteritems():
			language = self.getLibraryLanguage(library['machine_name'], library['major_version'], library['minor_version'], langageCode)
			if language != None:
	...
	def processParameters(self, contentId, newLibrary, newParameters, oldLibrary=None, oldParameters=None):
		...
		if oldLibrary != None:
			self.processSemantics(oldFiles, self.h5p.loadLibrarySemantics(oldLibrary['name'], oldLibrary['majorVersion'], oldLibrary['minorVersion'], oldParameters))
