...
class H5PEvent:
	...
	def saveStats(self):
		...
		currentNum = h5p_counters.objects.filter(type=atype, library_name=self.library_name, library_version=self.library_version).values('num')

		if not currentNum.exists():
			# Insert new counter
			h5p_counters.objects.create(type=atype, library_name=self.library_name, library_version=self.library_version, num=1)
		else:
			# Update counter with num+1
			h5p_counters.objects.filter(type=atype, library_name=self.library_name, library_version=self.library_version).update(num=F('num') + 1)
